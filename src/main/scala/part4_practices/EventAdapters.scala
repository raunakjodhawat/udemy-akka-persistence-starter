...
// WARNING: This file may not compiler or may fail to run
object EventAdapters extends App {
    ...
    override def fromJournal(event: Any, manifest: String): EventSeq =
      event match {
        case GuitarAdded(guitarId, guitarModel, guitarMake, quantity) =>
          EventSeq.single(
            GuitarAddedV2(guitarId, guitarModel, guitarMake, quantity, ACOUSTIC)
          )
  ...
  val system = ActorSystem(
    "eventAdaptors",
    ConfigFactory.load().getConfig("eventAdaptors")
  )
  val inventoryManager =
    system.actorOf(Props[InventoryManager], "inventoryManager")

  val guitars =
    for (i <- 1 to 10) yield Guitar(s"$i", s"Hakker $i", "RockTheJVM")
